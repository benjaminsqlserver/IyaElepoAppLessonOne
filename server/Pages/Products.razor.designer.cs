...
namespace IyaElepoApp.Pages
{
    public partial class ProductsComponent : ComponentBase
    {
        ...
        protected RadzenDataGrid<IyaElepoApp.Models.ConData.Product> grid0;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var conDataGetProductsResult = await ConData.GetProducts(new Query() { Filter = $@"i => i.ProductName.Contains(@0) || i.Picture.Contains(@1)", FilterParameters = new object[] { search, search }, Expand = "ProductType" });
        ...
        protected async System.Threading.Tasks.Task Splitbutton0Click(RadzenSplitButtonItem args)
        {
            if (args?.Value == "csv")
            {
                await ConData.ExportProductsToCSV(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "ProductType", Select = "ProductID,ProductName,ProductType.ProductTypeName as ProductTypeProductTypeName,Picture" }, $"Products");

            }

            if (args == null || args.Value == "xlsx")
            {
                await ConData.ExportProductsToExcel(new Query() { Filter = $@"{(string.IsNullOrEmpty(grid0.Query.Filter)? "true" : grid0.Query.Filter)}", OrderBy = $"{grid0.Query.OrderBy}", Expand = "ProductType", Select = "ProductID,ProductName,ProductType.ProductTypeName as ProductTypeProductTypeName,Picture" }, $"Products");

            }
        }

        protected async System.Threading.Tasks.Task Grid0RowSelect(IyaElepoApp.Models.ConData.Product args)
        {
