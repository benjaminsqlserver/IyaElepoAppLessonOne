...
{
    public partial class AddProductSupplierComponent : ComponentBase
    {
        ...
        {
            productsupplier = new IyaElepoApp.Models.ConData.ProductSupplier(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(IyaElepoApp.Models.ConData.ProductSupplier args)
        {
            try
            {
                var conDataCreateProductSupplierResult = await ConData.CreateProductSupplier(productsupplier);
                DialogService.Close(productsupplier);
            }
            catch (System.Exception conDataCreateProductSupplierException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new ProductSupplier!" });
            }
        }
